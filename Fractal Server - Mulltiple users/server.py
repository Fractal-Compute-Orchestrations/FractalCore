...
_SERVICE_ACCOUNT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "firebase_service_account.json"
)
...
SHARED_MODEL_DIR      = os.path.join(BASE_DIR, "downloads")
SHARED_MODEL_FILENAME = "0009_model.tflite"
# TENANTS_JSON          = os.path.join(BASE_DIR, "tenants.json")
...
class TenantSession:
    def __init__(self, username: str, n: int):
        ...
        self.download_dir     = os.path.join(BASE_DIR, "tenants", username, "bins")
        self.upload_dir       = os.path.join(BASE_DIR, "tenants", username, "uploads")
        self.global_ckpt_path = os.path.join(BASE_DIR, "tenants", username,
...
def api_delete_tenant(username):
    ...
    tenant_dir = os.path.join(BASE_DIR, "tenants", username)
