...
class IDRegistry:
    ...
    def generateTaskId(
        self, model_id: str, data_id: str, segment_sequence: str
    ) -> tuple:
        with self._lock:
...
def credit_mbs_for_device(device_id: str, mbs_to_add: float) -> bool:
    ...
    try:
        ...
        if current_balance >= MAX_CAP:
            add_log(
                f"Reward Skipped: {email} is already at MAX capacity (2GB).", "info"
            )
            return True
        ...
        user_ref.set({"liquid_mbs": Increment(final_increment)}, merge=True)
        add_log(
            f"Credited +{final_increment} MB -> {email}. (Total: ~{current_balance + final_increment}MB)",
            "success",
        )
        return True
    except Exception as e:
...
def perform_federated_averaging():
        ...
        try:
            ...
            for key in actual_keys:
                ...
                weight_accumulator[key] = (
                    tensor.copy()
                    if weight_accumulator[key] is None
    ...
    for key in actual_keys:
        ...
        tensor_values.append(
            tf.convert_to_tensor(weight_accumulator[key] / valid_count)
        )
    ...
    if server_state["round"] > CONFIG["MAX_ROUNDS"]:
        ...
        add_log(
            f"All {CONFIG['MAX_ROUNDS']} round(s) complete. Server finished.", "success"
        )
...
def create_android_ready_bins(n_bins, output_dir, items_per_bin, shuffle=True):
    ...
    if n_bins * items_per_bin > len(images):
        raise ValueError(
            f"Not enough data — requested {n_bins * items_per_bin}, have {len(images)}."
        )
    ...
    labels = to_categorical(labels, num_classes=CONFIG["NUM_CLASSES"]).astype(
        np.float32
    )
    ...
    for i in range(n_bins):
        ...
        if i == 0:
            add_log(
                f"Bin 0 -> images {(save_path/img_name).stat().st_size}B  "
                f"labels {(save_path/lbl_name).stat().st_size}B",
                "info",
            )
...
def get_current_task():
    ...
    if len(assigned_devices) >= CONFIG["MAX_CLIENTS"]:
        add_log(
            f"Device {device_id} rejected — cap of {CONFIG['MAX_CLIENTS']} reached.",
            "warn",
        )
        return (
            jsonify({"error": f"Max client limit ({CONFIG['MAX_CLIENTS']}) reached."}),
            403,
        )
    ...
    with task_queue_lock:
        if not task_queue:
            if _bin_segment_cache:
                ...
                add_log(
                    f"Device {device_id} requested a task — queue empty, no bin cache.",
                    "warn",
                )
    ...
    add_log(
        ...
        f"({len(assigned_devices)}/{CONFIG['MAX_CLIENTS']})",
        "success",
    )
    return jsonify(task)
...
def upload_checkpoint():
    task_Id = (
        request.form.get("task_Id") or request.form.get("taskId") or "unknown_task"
    )
    ...
    if dispatch_info:
        ...
        threading.Thread(
            target=credit_mbs_for_device, args=(target_device, reward_mbs), daemon=True
        ).start()
    else:
        add_log(
            f"upload_checkpoint: no dispatch record for task_Id {task_Id} — reward skipped.",
            "warn",
        )
    # ─────────────────────────────────────────────────────────────────────────
...
def get_status():
    tflite_path = os.path.join(
        BASE_DIR, "downloads", f"{CONFIG['MODEL_ID']}_model.tflite"
    )
    tflite_kb = (
